...
\end{equation}
By theorem \ref{EulerLagrangeThm}, the functional $F$ \eqref{F2} is stationary for $\phi$ satisfying \eqref{EulerLagrangeSol}.
...
where $+$ and $-$ denote the positive and the negative part respectively. Thus if the forward and the backward approximations of the derivatives yield different signs, the largest is chosen. In any other case, one of them will be 0. The equation \eqref{reinit2} is then updated by 
\begin{equation}
