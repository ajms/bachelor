...
\lhead{\cref{chapter:OL} \emph{Orderless Levelset}} % This is for the header on each page - perhaps a shortened title
...
The energy based functional \eqref{F1} can be formulated in terms of Locally Orderless Images. Let $I:\Omega\to \Gamma$ be the image and let $\phi:\Omega\to\R$ be the levelset function, where $\Omega\subset \R^k$ is the image domain and $\Gamma\subset\R$ are the intensity values. Notice that $\Gamma$, the intensity values, is a finite interval. Taking those functions into account, we formulate the joint histogram as in \cite{dar.11}:
...
with the Parzen estimate of the joint density: \textbf{Has to be corrected}.

\begin{equation}
  \bar{p}_{I,\phi}(i,j,\vec{x}_0,\alpha,\vec{\beta},\vec{\sigma}) = \frac{h_{I,\phi}(i,j,\vec{x}_0,\alpha,\vec{\beta},\vec{\sigma})}{\int_\Omega h_{I,\phi}(i,j,\vec{x}_0,\alpha,\vec{\beta},\vec{\sigma})\df \vec{x_0}}.
\end{equation}
...
The ``Locally Ordelerless'' property has no immediate use in the levelset method. So we introduce the Ordeless Levelset by letting $\alpha\to \infty$, the spatial window $W(\vec{x}_0,\alpha)$ will tend to 1 on the whole $\Omega$-domain. Thus the histogram simplifies to
...
Fix $\vec{\sigma}, \vec{\beta}, c_1, c_2$ and let $(\Gamma\times \R_+,\mathbb{B}^2(\R),m_2(i,j))$ be the $\sigma$-finite measure space on which $f(i,j) = \abs{i-c_1}h_{I,\phi}(i,j,\vec{\sigma},\vec{\beta})$ is defined. The Parzen window is per definition integrable, in particular measurable and $\abs{i-c_1}$ is measurable, since it is continuous. This implies that $f$ is measurable, because $f$ is the product of measurable functions. Since $f$ is positive, we can use Tonellis theorem \eqref{Tonelli}, thus
\begin{equation}
  \begin{split}
  ...
  \end{split}
\end{equation}
Furthermore, we can bound $\abs{i-c_1}$ by a constant $K$, because $\Gamma$ is assumed to be a bounded interval. This gives
\begin{equation}
  \begin{split}
    &\int\int 1_\Gamma(i) 1_{\R_+}(j) \abs{i-c_1}^2h_{I,\phi}(i,j,\vec{\beta},\vec{\sigma})\df m(i) \df m(j)\\
...
We are interested in minimizing the functional $F$, hence we find the gradient wrt. to $\phi$, since we wish to minimize $\phi$. Let $\partial_\phi$ denote the differential operator wrt. $\phi$. Thus the differential is given by
...
In order to find the derivative of the functional \eqref{LOLF}, we need to find the derivative of the Parzen window, as explaned in \ref{optimization}. The derivative is found to be
